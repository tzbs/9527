...
old_data = pd.DataFrame(columns=name)
for i in range(0,100,10):
    ...
    for list2 in list1:
        ...
        items.append(int(first_appmsg_info['read_num']))
        items.append(int(first_appmsg_info['like_num']))
        comment_num = first_appmsg_info.get('comment_num', 0)
        items.append(int(comment_num))
        article_info.append(items)
    new_data = pd.DataFrame(data=article_info)
        # 将新数据追加到旧数据上
    old_data=pd.concat([old_data, new_data], ignore_index=True)
    # 将结果写回CSV文件
    old_data.to_csv('wxgzpt.csv', index=False, encoding='UTF-8')
